...
public class ProfileDaoImpl implements Dao<Profile>{
        ...
        try {
            ...
            while (resultSet.next()) {
                Profile profile = new Profile(
                        ...
                        resultSet.getString("avatar_image"),
    ...
    public int updateModel(Profile profile, String oldEmail) throws SQLException {
        String sql = "UPDATE profile SET name = ?, login = ?, password = ?, birthday = ?, sex = ?, avatar_image = ?, about = ? WHERE email = ?;";
        PreparedStatement ps = connection.prepareStatement(sql);
        ...
        try {
            if(!profile.getAvatar_image().equals("")){
                ps.setBytes(6, Files.readAllBytes(Paths.get(profile.getAvatar_image())));
            }
            else{
                byte[] arr = new byte[0];
                ps.setBytes(6, arr);
    ...
    private static void setStrings(Profile profile, PreparedStatement ps) throws SQLException {
        ...
        try {
            if(!profile.getAvatar_image().equals("")){
                ps.setBytes(7, Files.readAllBytes(Paths.get(profile.getAvatar_image())));
            }
            else{
                byte[] arr = new byte[0];
                ps.setBytes(7, arr);
